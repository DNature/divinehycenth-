    ...
    <footer className="bg-g-25 mt-16" style={{ justifySelf: "flex-start" }}>
      <div className="container pt-12 pb-20 lg:grid grid-cols-8 gap-6">
        ...
        <div className="col-span-2  text-center lg:text-left mt-8 lg:mt-0">
          ...
          <CustomLink className={`font-bold block hover:text-g-75 ${pathname == "/" && "text-g-75"}`} href={"/"}>
            Home
          </CustomLink>
          {routes.map(({ path, name }) => (
            <CustomLink key={name} className={`font-bold block hover:text-g-75 ${pathname?.split("/")[1] == path.split("/")[1] && "text-g-75"}`} href={path}>
              {name}
            </CustomLink>
          ))}
        </div>
